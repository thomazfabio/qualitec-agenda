    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-container>
          ...
          <v-card-text class="pb-0" v-if="dataUser.name == 'nome'">
            <v-row>
              <v-col>
                <v-text-field
                  label="Seu nome*"
                  v-model="dataEdited.firstNane"
                  :placeholder="dataUser.value.split(' ')[0]"
                  outlined
                  dense
                  required
                ></v-text-field>
                <v-text-field
                  label="Seu sobrenome*"
                  v-model="dataEdited.lastName"
                  :placeholder="dataUser.value.split(' ').slice(1).join(' ')"
                  outlined
                  dense
                  required
                ></v-text-field>
          ...
          <v-card-text class="pb-0" v-if="dataUser.name == 'telefone'">
            <v-row>
              <v-col>
                <v-text-field
                  v-model="dataEdited.cellPhone"
                  name="input-cell-phone"
        ...
        <v-card-actions class="pt-0 pr-5">
          ...
          <div v-else>
            <v-btn
              color="personal_action_2 darken-1"
              text
            ...
            <v-btn
              color="personal_action_1 darken-1"
              text
